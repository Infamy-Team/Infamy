...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
    ...
    public class AeroplaneAiControl : MonoBehaviour
    {
        ...
        // Codigo Propio
        public enum Tipo { Aliado=0, Enemigo };
        [SerializeField] private Tipo m_Tipo;
        [SerializeField] private Transform m_SecondTarget;

        void OnTriggerEnter(Collider col)
        {
            if (col.tag == "EnemigoBarco" && m_Tipo == Tipo.Aliado)
            {
                if (this.GetComponent<UnityStandardAssets.Vehicles.Aeroplane.AeroplaneController>().Preparado())
                    this.GetComponent<UnityStandardAssets.Vehicles.Aeroplane.AeroplaneController>().Disparar();
                if (this.GetComponent<UnityStandardAssets.Vehicles.Aeroplane.AeroplaneController>().m_cantidadBombas <= 0)
                {
        ...
        void update()
        {
            if (this.CompareTag("Aliado") && m_Target == null)
            {
                SetTarget(m_SecondTarget);
            }
        }
