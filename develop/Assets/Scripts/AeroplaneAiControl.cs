    ...
    public class AeroplaneAiControl : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            if (m_Target != null)
            {
                ...
                #region Distancia Metralleta
                float dist = Vector3.Distance(this.transform.position, m_Target.position);


                if (dist > 180 && this.GetComponent<Objetos.AeroplaneController>().MaxEnginePower != 100)
                {
                    this.GetComponent<Objetos.AeroplaneController>().CambiarVel(100);
                    this.GetComponent<Objetos.AeroplaneController>().m_disparando = false;
                }
                else if (dist <= 180 && !this.GetComponent<Objetos.AeroplaneController>().m_disparando && this.tag == "Enemigo")
                {
                    Debug.Log("Pium Pium");
                    if (this.GetComponent<Objetos.AeroplaneController>().MaxEnginePower != 180)
                        this.GetComponent<Objetos.AeroplaneController>().CambiarVel(60);
        ...
        // Codigo Propio
        //[SerializeField] private Transform m_SecondTarget;

        
        void OnTriggerEnter(Collider col)
        {
            if (col.tag == "Enemigo" && this.tag == "Aliado" && this.GetComponent<Objetos.AeroplaneController>().Preparado())
            {
                this.GetComponent<Objetos.AeroplaneController>().DispararBomba();
                /*
                if (this.GetComponent<Objetos.AeroplaneController>().m_cantidadBombas <= 0)
                {
                    SetTarget(m_SecondTarget);
                }
                */
            }
        }
        ...
        void Update()
        {
            if (this.CompareTag("Aliado") && !m_Target)
            {
                if(Application.loadedLevelName == "Menu")
                    m_Target = GameObject.Find("Sphere").GetComponent<Manager>().GetRandomTarget();
                else
                    m_Target = GameObject.Find("Player").GetComponent<Manager>().GetRandomTarget();
            }
        }
    }
}
